...
class OptionValue:
    ...
    def __init__(self, cookies: Optional[Dict[str, str]] = None):
        ...
        self.default_fields = "f1,f2,f3,f12,f13,f14,f298,f299,f249,f300,f330,f331,f332,f333,f334,f335,f336,f301,f152"
        self.timeout = 10
        self.retry_times = 3
...
或者使用浏览器扩展如"EditThisCookie"直接导出cookies
    ...
    def get_data(
        ...
        返回:
            pandas.DataFrame: 包含期权价值数据的DataFrame
        """
        params = {
            "fid": sort_field,
            "po": sort_direction,
        ...
        for attempt in range(self.retry_times):
            try:
                ...
                # 计算价值比例
                if "价值比例" in df.columns:
                    df["价值比例"] = df["价值比例"] / 100
                elif "f300" in df.columns:
                    df["f300"] = df["f300"] / 100
    ...
    def get_etf_options_data(
        ...
        返回:
            pandas.DataFrame: 包含主要ETF期权价值数据的DataFrame
        """
        # 获取ETF市场数据
...
if __name__ == "__main__":
    ...
    value = OptionValue(cookies=cookies)
    
    print("="*50)
    print("东方财富期权价值数据接口使用示例")
    print("="*50)
    ...
    try:
        ...
        # 获取多页数据
        print("\n2. 获取多页ETF期权价值数据:")
        df_multi = value.get_multi_pages(max_pages=10)
        print(f"获取成功, 总数据量: {len(df_multi)}")
        print(df_multi.head())
        
        # 获取主要ETF期权数据
        print("\n3. 获取主要ETF期权价值数据:")
        df_etf = value.get_etf_options_data()
