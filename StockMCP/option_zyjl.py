...
class OptionZYJL:
    ...
    def __init__(self, cookies: Optional[Dict[str, str]] = None):
        ...
        self.default_fields = "f1,f2,f3,f12,f13,f14,f161,f250,f330,f331,f332,f333,f334,f335,f337,f301,f152"
        self.timeout = 10
        self.retry_times = 3
...
或者使用浏览器扩展如"EditThisCookie"直接导出cookies
    ...
    def get_data(
        ...
        返回:
            pandas.DataFrame: 包含期权折溢价率数据的DataFrame
        """
        params = {
            "fid": sort_field,
            "po": sort_direction,
    ...
    def get_etf_options_data(
        ...
        返回:
            pandas.DataFrame: 包含主要ETF期权折溢价率数据的DataFrame
        """
        # 获取ETF市场数据
...
if __name__ == "__main__":
    ...
    zyjl = OptionZYJL(cookies=cookies)
    
    print("="*50)
    print("东方财富期权折溢价率数据接口使用示例")
    print("="*50)
    ...
    try:
        ...
        # 获取多页数据
        print("\n2. 获取多页ETF期权折溢价率数据:")
        df_multi = zyjl.get_multi_pages(max_pages=3)
        print(f"获取成功, 总数据量: {len(df_multi)}")
        print(df_multi.head())
        
        # 获取主要ETF期权数据
        print("\n3. 获取主要ETF期权折溢价率数据:")
        df_etf = zyjl.get_etf_options_data()
